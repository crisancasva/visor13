<div class="container-fluid mt-2">
    ...
    ?>
    <h2 class="text-center display-4" style="margin-top: 5px; margin-bottom: 15px;">Solicitudes de vía publica en mal estado</h2>
    <?php
        }else{
    ?>
    <h2 class="text-center display-4" style="margin-top: 5px; margin-bottom: 15px;">Mis solicitudes de vía publica en mal estado</h2>
    <?php
        }
    ?>
            ...
            <tbody>
                <?php
                    ...
                    foreach($viamal as $via){
                        echo "<tr>";
                        if($_SESSION['rolcod']===2 || $_SESSION['rolcod']===1){
                            echo "<td>". $via['vmecod']."</td>";
                        }
                        echo "<td>". $via['vmedescripcion']."</td>";
                        echo "<td><img src='". $via['vmeimagen']."'width='60px'></td>";
                        echo "<td>". $via['vmedireccion']."</td>";
                        echo "<td>". $via['vmefecha']."</td>";
                        echo "<td>". $via['vmeobservacion']."</td>";
                        echo "<td>". $via['estdescripcion']."</td>";
                        if($_SESSION['rolcod']===2 || $_SESSION['rolcod']===1){
                            echo "<td>". $via['usunombrecompleto']."</td>";
                        }
                        echo "<td>". $via['tdanodescripcion']."</td>";
                        if($_SESSION['rolcod']===2 || $_SESSION['rolcod']===1){
                            echo "<td><button class='btn' style='background:#4E74A6 !important; color:white;' onclick=\"window.location.href='" . getUrl('Solicitud', 'MallaVialMalEstado', 'getMallaVialMalEstadoUpdate', array('vmecod' => $via['vmecod'])) . "'\">Editar</button></td>";
                        }
                        echo "</tr>";
                    }
                ?>
                <tr id="filaSinResultados" style="display: none;">
                    <td colspan="10" class="text-center text-danger">Ningún resultado coincide con los criterios de búsqueda ingresados.</td>
                </tr>
            </tbody>      
        </table>
        <?php
...
<script src="../web/assets/js/solicitud/mallaVialMalEstadoConsul.js"></script>