<div class="container-fluid mt-2">
    ...
    ?>
    <h2 class="text-center display-4" style="margin-top: 5px; margin-bottom: 15px;">Solicitudes de nuevas señalizaciones</h2>
    <?php
    }else{
    ?>
    <h2 class="text-center display-4" style="margin-top: 5px; margin-bottom: 15px;">Mis solicitudes de nuevas señalizaciones</h2>
    <?php
    }
    ?>
    ...
    <table id = "tablaConsultar" class="table table-striped table-hover">
        <thead style="position: sticky; top: 0; background: #fff; z-index: 100;">
            ...
            <tbody>
                <?php
                    foreach($senalizacion as $sena){
                        echo "<tr>";
                        if($_SESSION['rolcod']===2 || $_SESSION['rolcod']===1){
                            echo "<td>". $sena['nscod']."</td>";
                        }
                        
                        echo "<td>". $sena['nsdescripcion']."</td>";
                        echo "<td><img src='". $sena['nsimagen']."'width='60px'></td>";
                        echo "<td>". $sena['nsdireccion']."</td>";
                        echo "<td>". $sena['nsfecha']."</td>";
                        echo "<td>". $sena['nsobservacion']."</td>";
                        ...
                        if($_SESSION['rolcod']===2 ||$_SESSION['rolcod']===1){
                        echo "<td><button class='btn' style='background:#4E74A6 !important; color:white;' onclick=\"window.location.href='" . getUrl('Solicitud', 'NuevaSenal', 'getUpdateNuevaSenal', array('nscod' => $sena['nscod'])) . "'\">Editar</button></td>";
                        }
                        echo "</tr>";
                    }
                ?>
...
<script src="../web/assets/js/solicitud/nuevaSenalConsul.js"></script>